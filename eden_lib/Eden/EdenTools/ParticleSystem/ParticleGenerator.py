...
class ParticleGenerator:
    ...
    def loadDriver(self, fileName):
        ...
        try:
            ...
            print("Failed to load particle driver file: ", repr(fileName))
            raise
    ...
    def loadXMLConfig(self, xmlParticle, extraData=None):
        ...
        # ---------------------------------------------------------------------
        t_xpd["pos"] = (
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["posX"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["posY"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["posZ"],
        )
        t_xpd["hpr"] = (
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["genH"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["genP"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["genR"],
        )
        t_xpd["scale"] = (
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["scaleX"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["scaleY"],
            self.particleXPU.Parser["XML_Values"]["Generator_Values"]["scaleZ"],
        )
        # ---------------------------------------------------------------------
        # GENERAL PARTICLES PARAMETERS
        # ---------------------------------------------------------------------
        for t_x in self.particleXPU.Parser["XML_Values"]["General_Values"]:
            t_xpd[t_x] = self.particleXPU.Parser["XML_Values"]["General_Values"][t_x]
        # ---------------------------------------------------------------------
        # FACTORY PARAMETERS
        # ---------------------------------------------------------------------
    ...
    def genericDriver(self, effectData):
        ...
        self.rootGenerator.setPos(
            effectData["pos"][0], effectData["pos"][1], effectData["pos"][2]
        )
        self.rootGenerator.setHpr(
            effectData["hpr"][0], effectData["hpr"][1], effectData["hpr"][2]
        )
        self.rootGenerator.setScale(
            effectData["scale"][0], effectData["scale"][1], effectData["scale"][2]
        )
        ...
        if effectData["renderer"] == "PointParticleRenderer":
            ...
            t_ps.renderer.setStartColor(
                Vec4(
                    effectData["r"]["startColR"],
                    effectData["r"]["startColG"],
                    effectData["r"]["startColB"],
                    effectData["r"]["startColA"],
                )
            )
            t_ps.renderer.setEndColor(
                Vec4(
                    effectData["r"]["endColR"],
                    effectData["r"]["endColG"],
                    effectData["r"]["endColB"],
                    effectData["r"]["endColA"],
                )
            )
            t_ps.renderer.setBlendType(blendModeList[effectData["r"]["blendType"]])
            t_ps.renderer.setBlendMethod(
                interpolModeList[effectData["r"]["blendMethod"]]
            )
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
            ...
            # we have to get the unique parameters
            t_ps.renderer.setHeadColor(
                Vec4(
                    effectData["r"]["headColR"][0],
                    effectData["r"]["headColG"][1],
                    effectData["r"]["headColB"][2],
                    effectData["r"]["headColA"][3],
                )
            )
            t_ps.renderer.setTailColor(
                Vec4(
                    effectData["r"]["tailColR"],
                    effectData["r"]["tailColG"],
                    effectData["r"]["tailColB"],
        ...
        # ---------------------------------------------------------------------
            ...
            # we have to get the unique parameters
            t_ps.renderer.setCenterColor(
                Vec4(
                    effectData["r"]["centerColR"],
                    effectData["r"]["centerColG"],
                    effectData["r"]["centerColB"],
                    effectData["r"]["centerColA"],
                )
            )
            t_ps.renderer.setEdgeColor(
                Vec4(
                    effectData["r"]["edgeColR"],
                    effectData["r"]["edgeColG"],
                    effectData["r"]["edgeColB"],
        ...
        # ---------------------------------------------------------------------
            ...
            t_ps.renderer.setAlphaBlendMethod(
                interpolModeList[effectData["r"]["blendMethod"]]
            )
        ...
        t_ps.emitter.setExplicitLaunchVector(
            Vec3(
                effectData["e"]["elvX"],
                effectData["e"]["elvY"],
                effectData["e"]["elvZ"],
        ...
        if effectData["emitter"] == "BoxEmitter":
            # we have to get the unique parameters
            t_ps.emitter.setMinBound(
                Vec3(
                    effectData["e"]["minBoundX"],
                    effectData["e"]["minBoundY"],
                    effectData["e"]["minBoundZ"],
                )
            )
            t_ps.emitter.setMaxBound(
                Vec3(
                    effectData["e"]["maxBoundX"],
                    effectData["e"]["maxBoundY"],
        ...
        # ---------------------------------------------------------------------
            ...
            # we have to get the unique parameters
            t_ps.emitter.setLocation(
                Vec3(
                    effectData["e"]["locationX"],
                    effectData["e"]["locationY"],
                    effectData["e"]["locationZ"],
                )
            )
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
            ...
            # we have to get the unique parameters
            t_ps.emitter.setMinBound(
                effectData["e"]["minBoundX"], effectData["e"]["minBoundZ"]
            )
            t_ps.emitter.setMaxBound(
                effectData["e"]["maxBoundX"], effectData["e"]["maxBoundZ"]
            )
        # ---------------------------------------------------------------------
        ...
        if effectData["x"]["nullSection"] != True:
            ...
            for t_h in range(effectData["x"]["forceCount"]):
                ...
                t_f1 = forceList[t_b](
                    self.particleXPU.Parser["XML_Values"][t_a]["forceX"],
                    self.particleXPU.Parser["XML_Values"][t_a]["forceY"],
                    self.particleXPU.Parser["XML_Values"][t_a]["forceZ"],
                )
                # set the force parameters
                t_f1.setActive(
                    self.particleXPU.Parser["XML_Values"][t_a]["setForceActive"]
                )
