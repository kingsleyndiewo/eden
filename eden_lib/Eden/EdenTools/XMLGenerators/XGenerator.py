...
class XGenerator:
    ...
    def createSection(self, tagID, tagName, tagValue, appendToBase=True, parent=None):
        ...
        if appendToBase == True:
            ...
            self.elementStore["baseElement"].appendChild(
                self.elementStore["sections"][tagValue]
            )
        else:
            # the section is nested in another section
            self.elementStore["sections"][parent].appendChild(
                self.elementStore["sections"][tagValue]
            )
    ...
    def createSectionWithValues(
        self, tagID, tagName, tagValue, valueList, appendToBase=True, parent=None
    ):
