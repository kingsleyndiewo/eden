...
class Visuals2D:
    ...
    def showImage(self, listIndex):
        ...
        self.Surface = OnscreenImage(
            image=self.media["images"][listIndex][0],
            pos=self.media["images"][listIndex][1],
            scale=self.aspectScale,
        )
        if self.virginFlag != False:
    ...
    def switcherTask(self, task):
        ...
        if task.time == 0.0:
            ...
            return task.cont
        elif (task.frame / self.soloRegister[task.name][2]) == self.soloRegister[
            task.name
        ][0]:
            # time for next image
            self.soloRegister[task.name][2] += 1
