...
class Text2D:
    ...
    def displayText(self, textKey, intervalSec=0, textPosition=None, duplicates=True):
    ...
    def showText(self, textKey, cluster=False):
        ...
        t_n.setScale(
            self.texts[textKey][1]["Scale"] * self.texts[textKey][1]["FontSize"]
        )
        # set the position
    ...
    def switcherTask(self, task):
        ...
        if task.time == 0.0:
            ...
            return task.cont
        elif (task.frame / self.soloRegister[task.name][3]) == self.soloRegister[
            task.name
        ][1]:
            # time for next text
            self.soloRegister[task.name][3] += 1
            if len(self.soloRegister[task.name][0]) >= self.soloRegister[task.name][3]:
                # safe to show, text exists
                self.switchText(
                    self.soloRegister[task.name][0][self.soloRegister[task.name][3] - 1]
                )
                return task.cont
            else:
                # out of text!
