...
class Genesis(Creation):
    ...
    def setupActorCollision(self, actorName):
        ...
        if self.handlerType == 0:
            ...
            self.edenColHandler.addCollider(
                self.actorStore[actorName].actorCnode,
        ...
        if t_c != 1:
            ...
            self.edenTraverser.addCollider(
                self.actorStore[actorName].actorCnode, self.edenColHandler
            )
    ...
    def setupObjectCollision(self, objectName):
        ...
        if objectName not in self.objectStore:
            ...
            t_s = CollisionSphere(
                self.objectStore[objectName][2]["center"],
            ...
            self.objectStore[objectName][2]["colNode"] = self.objectStore[objectName][
                0
            ].attachNewNode(t_Cnode)
            # all objects have collider type 1
            self.objectStore[objectName][2]["colNode"].node().setFromCollideMask(
                BitMask32.allOff()
            )
            self.objectStore[objectName][2]["colNode"].setCollideMask(
                BitMask32.bit(self.geomBitMask)
            )
    ...
    def loadInventory(self, invFile, inventoryName):
        ...
        for t_y in t_l:
            ...
            t_scl = (
                invXPU.Parser["XML_Values"][t_q]["scaleX"],
            ...
            t_pos = (
                invXPU.Parser["XML_Values"][t_q]["posX"],
            ...
            t_hpr = (
                invXPU.Parser["XML_Values"][t_q]["itemH"],
    ...
    def loadActor(self, actorDetailsFile, overrideName=None):
        ...
        actorDetails["scale"] = (
            actorXPU.Parser["XML_Values"]["Main_Values"]["scaleX"],
            actorXPU.Parser["XML_Values"]["Main_Values"]["scaleY"],
        ...
        actorDetails["position"] = (
            actorXPU.Parser["XML_Values"]["Setup_Values"]["positionX"],
            actorXPU.Parser["XML_Values"]["Setup_Values"]["positionY"],
        ...
        for t_nv in t_ncl:
            if t_nv in actorXPU.Parser["XML_Values"]["Setup_Values"]:
                ...
                actorDetails[t_nv] = actorXPU.Parser["XML_Values"]["Setup_Values"][t_nv]
        # check whether this is a tracked actor
        if actorXPU.Parser["XML_Values"]["Setup_Values"]["isTracked"] == 1:
            actorDetails["camPosition"] = [
                actorXPU.Parser["XML_Values"]["Setup_Values"]["cameraX"],
                actorXPU.Parser["XML_Values"]["Setup_Values"]["cameraY"],
        ...
        if actorXPU.Parser["XML_Values"]["Setup_Values"]["actorType"] == 0:
            self.actorStore[actorDetails["name"]] = EdenWalker(
                actorDetails, self.actorsNode
            )
        elif actorDetails["actorType"] == 1:
            self.actorStore[actorDetails["name"]] = EdenAero(
                actorDetails, self.actorsNode
            )
        else:
        ...
        if actorXPU.Parser["XML_Values"]["Setup_Values"]["detectTerrain"] == 1:
            ...
            t_tcn = self.actorStore[actorDetails["name"]].baseActor.attachNewNode(
                CollisionNode(actorDetails["name"] + "_TCN")
            )
            ...
            if actorXPU.Parser["XML_Values"]["Setup_Values"]["enablePhysics"] == 0:
                ...
                self.terrainDetector.addCollider(
                    t_tcn, self.actorStore[actorDetails["name"]].baseActor
                )
            else:
                # check if the world has physics capability
                if self.worldData["enablePhysics"] != 1:
                    ...
                    self.terrainDetector.addCollider(
                        t_tcn, self.physicsStore["actors"][actorDetails["name"]][0]
                    )
    ...
    def setActorMass(self, actorName, newMass):
        ...
        if actorName not in self.actorStore:
            ...
            self.physicsStore["actors"][actorName][1].getPhysicsObject().setMass(
                newMass
            )
    ...
    def attachForceToActor(self, actorName, forceName):
        ...
        if actorName not in self.actorStore:
            ...
            if self.physicsStore["forces"][forceName][1] != 2:
                # a linear force
                self.physicsStore["actors"][actorName][1].getPhysical(0).addLinearForce(
                    self.physicsStore["forces"][forceName][0]
                )
            else:
                # an angular force
                self.physicsStore["actors"][actorName][1].getPhysical(
                    0
    ...
    def attachJetPackToActor(
        self, forceName, jetPack, actorName, packPos=(0, 0, 0), packHpr=None
    ):
        ...
        if actorName not in self.actorStore:
            ...
            if packHpr == None:
                ...
                jetPack.setHpr(
                    self.actorStore[actorName].baseActor.getH(),
    ...
    def keyboardSetup(self):
        ...
        if self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["enableModifiers"] == 1:
    ...
    def defaultAnimationTask(self, task):
        ...
        if True in self.controller.values():
            ...
            if self.mainActor.baseActor.getTag("Animated") == "True":
                ...
                self.mainActor.baseActor.pose(
                    self.anime, self.mainActor.actorData["pose"]
                )
    ...
    def updateViewTask(self, task):
        ...
        if True not in self.controller.values():
            ...
            if t_az < t_wz:
                t_az = t_wz
            base.camera.setPos(
                self.mainActor.baseActor.getX() + self.camPos[0],
