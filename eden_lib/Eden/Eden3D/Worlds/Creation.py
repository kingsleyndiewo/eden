...
class Creation(ShowBase):
    ...
    def __init__(self, remoteStarterTask=None, customPRC=None, edenClass="Creation"):
        ...
        if self.camfixFlag == True:
            ...
            base.camera.setPos(
                self.worldData["cameraPos"][0],
                self.worldData["cameraPos"][1],
                self.worldData["cameraPos"][2],
            )
            base.camera.setHpr(
                self.worldData["cameraHpr"][0],
                self.worldData["cameraHpr"][1],
                self.worldData["cameraHpr"][2],
            )
    ...
    def loadGeometry(
        ...
        if extraData["enablePhysics"] == False:
            ...
            if extraData["objectMass"] != 0:
                ...
                self.setObjectMass(
                    modelName, extraData["objectMass"], extraData["odeMass"]
                )
            ...
            if (
                extraData["terrainDetection"] == True
                and self.physicsEngine == self.enginesList[0]
            ):
                ...
                t_tcn = self.objectStore[modelName][0].attachNewNode(
                    CollisionNode(modelName + "_TCN")
                )
                ...
                if extraData["enablePhysics"] == False:
                    ...
                    self.terrainDetector.addCollider(
                        t_tcn, self.objectStore[modelName][0]
                    )
                else:
                    # CollisionHandlerGravity
                    self.terrainDetector.addCollider(
                        t_tcn, self.physicsStore["geometry"][modelName][1]
                    )
    ...
    def makeTerrain(self, terrainXML):
        ...
        if (
            terraXPU.Parser["XML_Values"]["terrainDetails_Values"]["focalPointNode"]
            == 0
        ):
        ...
        if terraXPU.Parser["XML_Values"]["terrainDetails_Values"]["textured"] == 1:
            # build a path to the filename
            t_j = (
                self.gameMVC.mvcStructure["tier_resource"]["images"]
                + "/"
                + terraXPU.Parser["XML_Values"]["terrainDetails_Values"]["baseTexture"]
            )
            # load the texture
            t_d["baseTexture"] = loader.loadTexture(t_j)
        # set the path for textures
        t_d["terrainPath"] = (
            self.gameMVC.mvcStructure["tier_resource"]["images"] + "/terrain/"
        )
        # make the terrain
    ...
    def recordSIVideo(
        ...
        self.edenVisuals.saveSequencedImages(
            imageFilePrefix,
            recordDuration,
            framesPerSecond,
            videoFolder,
    ...
    def setSoundPan(self, jukeKey, newPan=0.0):
        ...
        if jukeKey not in self.jukeBox:
            ...
            if newPan < -1.0:
                ...
                newPan = 1.0
            # apply the new pan
            self.jukeBox[jukeKey].setBalance(newPan)
    ...
    def loadParticlesFile(self, particleFile, pGenName, fileSource=0, fileType=0):
    ...
    def initPhysics(self):
        ...
        self.physicsEngine = self.XPU.Parser["XML_Values"]["Physics_Values"][
            "physicsEngine"
        ]
        if self.physicsEngine == self.enginesList[0]:
            ...
            if "extraManager" in self.XPU.Parser["XML_Values"]["Physics_Values"]:
                ...
                if self.XPU.Parser["XML_Values"]["Physics_Values"]["extraManager"] == 1:
        ...
        elif self.XPU.Parser["XML_Values"]["Physics_Values"]["globalGravity"] == 2:
        ...
        if objectName not in self.objectStore:
            ...
            if self.physicsEngine == self.enginesList[0]:
                ...
                if odeData["odeMass"]["collisionShape"] == 0:
                    ...
                    t_cg = OdeCappedCylinderGeom(
                        self.odeSpace,
                        odeData["odeMass"]["radius"],
                        odeData["odeMass"]["length"],
                    )
                elif odeData["odeMass"]["collisionShape"] == 2:
                    t_cg = OdeCylinderGeom(
                        self.odeSpace,
                        odeData["odeMass"]["radius"],
                        odeData["odeMass"]["length"],
                    )
                    ...
                    t_cg = OdePlaneGeom(
                        self.odeSpace,
                        Vec4(
                            odeData["odeMass"]["vector"][0],
                            odeData["odeMass"]["vector"][1],
                            odeData["odeMass"]["vector"][2],
                            odeData["odeMass"]["vector"][3],
                        ),
                    )
                    ...
                    t_cg = OdeTriMeshGeom(
                        self.odeSpace, odeData["odeMass"]["triMeshData"]
                    )
    ...
    def setObjectMass(self, objectName, newMass, odeMassData=None):
        ...
        if objectName not in self.objectStore:
            ...
            if self.physicsEngine == self.enginesList[0]:
                # Panda physics
                self.physicsStore["geometry"][objectName][1].getPhysicsObject().setMass(
                    newMass
                )
            elif self.physicsEngine == self.enginesList[1]:
                # ODE physics
    ...
    def loadODESettings(self):
        ...
        if (
            self.odeXPU.Parser["XML_Values"]["Settings_Values"][
                "enableTerrainDetection"
            ]
            == True
        ):
        ...
        self.odeWorld.initSurfaceTable(
            self.odeXPU.Parser["XML_Values"]["Settings_Values"]["surfaceCount"]
        )
        ...
        for t_x in t_sl:
            ...
            self.odeWorld.setSurfaceEntry(
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["surfaceID1"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["surfaceID2"],
                t_mu,
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["bounce"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["bounceVelocity"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["softERP"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["softCFM"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["CDF"],
                self.odeXPU.Parser["XML_Values"]["%s_Values" % t_x]["dampingFactor"],
            )
    ...
    def createODEMass(self, massType, massValue, odeMassData):
        ...
        if massType == 0:
            ...
            if odeMassData["density"] == 0.0:
                # we use the argued total mass
                t_am.setBoxTotal(
                    massValue, odeMassData["X"], odeMassData["Y"], odeMassData["Z"]
                )
            else:
                # use the density
                t_am.setBox(
            ...
            if odeMassData["density"] == 0.0:
                # we use the argued total mass
                t_am.setCylinderTotal(
                    massValue,
                    odeMassData["direction"],
                    odeMassData["r"],
                    odeMassData["l"],
                )
            else:
    ...
    def loadForcesfromXML(self, forceXML, procList=[None]):
        ...
        for t_x in t_l:
            ...
            t_xyz = (
                self.forceXPU.Parser["XML_Values"][t_k]["forceX"],
            ...
            if t_t == 9:
                ...
                t_ea["UDFF"] = procList[self.forceXPU.Parser["XML_Values"][t_k]["UDFF"]]
            # create the force
            self.createForce(t_n, t_t, t_xyz, t_ea, t_g)

    def createForce(
        self, forceName, forceType, forceXYZ, uniqueArgs={}, setGlobal=False
    ):
    ...
    def createLinearVectorForce(self, forceName, forceXYZ, massDependence, setGlobal):
        "creates a new LVF; developer convenience front for createForce"
        # create the force
        self.createForce(
            forceName, 1, forceXYZ, {"massDependence": massDependence}, setGlobal
        )
    ...
    def createUniversalForce(self, forceName):
        ...
        if self.worldData["enablePhysics"] != 1:
            ...
            if self.physicsStore["forces"][forceName][1] != 2:
                # a linear force
                self.physicsManager.addLinearForce(
                    self.physicsStore["forces"][forceName][0]
                )
            else:
                # an angular force
                self.physicsManager.addAngularForce(
                    self.physicsStore["forces"][forceName][0]
                )
    ...
    def attachForceToObject(self, objectName, forceName):
        ...
        if objectName not in self.objectStore:
            ...
            if self.physicsStore["forces"][forceName][1] != 2:
                # a linear force
                self.physicsStore["geometry"][objectName][1].getPhysical(
                    0
                ).addLinearForce(self.physicsStore["forces"][forceName][0])
            else:
                # an angular force
                self.physicsStore["geometry"][objectName][1].getPhysical(
                    0
                ).addAngularForce(self.physicsStore["forces"][forceName][0])
    ...
    def removeForceFromObject(self, objectName, forceName):
        ...
        if objectName not in self.objectStore:
            ...
            if self.physicsStore["forces"][forceName][1] != 2:
                ...
                self.physicsStore["geometry"][objectName][1].getPhysical(
                    0
                ).removeLinearForce(self.physicsStore["forces"][forceName][0])
            else:
                # an angular force
                self.physicsStore["geometry"][objectName][1].getPhysical(
                    0
                ).removeAngularForce(self.physicsStore["forces"][forceName][0])
    ...
    def attachJetPackToObject(
        self, forceName, jetPack, objectName, packPos=(0, 0, 0), packHpr=None
    ):
        ...
        if objectName not in self.objectStore:
            ...
            if packHpr == None:
                ...
                jetPack.setHpr(
                    self.objectStore[objectName][0].getH(),
    ...
    def getPointBelowCursor(self, zValue, cursorPoint, cursorVec):
        ...
        return cursorPoint + cursorVec * (
            (zValue - cursorPoint.getZ()) / cursorVec.getZ()
        )
    ...
    def parseConfigXML(self, configXML):
        ...
        for t_y in t_g:
            t_x = (
                t_y + " : " + self.XPU.Parser["XML_Values"]["About_Values"][t_y] + "\n"
            )
            self.gameID += t_x
        # add the renderer
        ...
        # generated
        if self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["worldType"] == 0:
            # save the world path
            ...
            t_m = self.gameMVC.mvcStructure["config"] + "/"
            t_m += self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["terrainData"]
            self.worldData["worldPath"] = t_m
            self.worldData["isGenerated"] = True
        # get the world scaling
        self.worldData["worldScale"] = (
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["scaleX"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["scaleY"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["scaleZ"],
        )
        # get the world initial position
        self.worldData["worldPosition"] = (
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["posX"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["posY"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["posZ"],
        )
        ...
        if self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["cameraDefaults"] != 1:
            # get camera initialization data
            self.XPU.getSectionValues("subsection", "Camera")
            if self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraTrait"] == "FIXED":
                # save the camera data
                self.worldData["cameraPos"] = (
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraX"],
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraY"],
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraZ"],
                )
                self.worldData["cameraHpr"] = (
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraH"],
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraP"],
                    self.XPU.Parser["XML_Values"]["Camera_Values"]["cameraR"],
                )
        ...
        self.edenVisuals = Visuals2D(
            messenger.send,
            self.worldData["defaultFPS"],
    ...
    def loadStartupVisuals(self, tagID, tagValue):
        ...
        for t_j in t_u:
            ...
            if t_a[0] == "screen":
                ...
                t_r = self.XPU.Parser["XML_Values"][t_p][t_j].split(":")
                t_m += t_r[0]
                # get the position
        ...
        # store the delay
        self.visualDelay = self.XPU.Parser["XML_Values"][t_p]["interval"]
        self.mediaFlag[0] = 1
    ...
    def loadStartupSound(self, tagID, tagValue):
        ...
        self.worldData["playAfter"] = self.XPU.Parser["XML_Values"][t_p]["follows"]
        self.mediaFlag[1] = 1
    ...
    def worldClockTask(self, task):
        ...
        if (task.frame % self.edenVisuals.fpsValue) == 0:
            ...
            # compute the strings
            # local
            self.worldData["localTime"] = strftime("%H:%M:%S", t_lcl)
            # UTC
            self.worldData["zuluTime"] = strftime("%H:%M:%S", t_utc)
        return task.cont
    ...
    def odeSimulationTask(self, task):
        ...
        for t_x in self.physicsStore["geometry"]:
            self.objectStore[t_x][0].setPosQuat(
                render,
                self.physicsStore["geometry"][t_x][0].getPosition(),
                Quat(self.physicsStore["geometry"][t_x][0].getQuaternion()),
            )
