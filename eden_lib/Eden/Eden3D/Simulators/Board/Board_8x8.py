...
class Board_8x8(Creation):
    ...
    def __init__(
        ...
        self.autoFlip = self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["autoFlip"]
        ...
        self.boardSpans = (
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["row1"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["column1"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["row8"],
            self.XPU.Parser["XML_Values"]["WorldDetails_Values"]["column8"],
        )
        ...
        self.pieceScale = (
            self.XPU.Parser["XML_Values"]["Pieces_Values"]["scaleX"],
        ...
        if ignorePicker == False:
            ...
            t_d = {
                "CNF": base.camera.attachNewNode,
                "MWN": base.mouseWatcherNode.getMouse,
                "msgr": messenger.send,
                "BCNF": base.camNode,
                "parentNode": self.world,
            }
    ...
    def resetView(self):
        ...
        base.camera.setHpr(
            self.defaultView[0], self.defaultView[1], self.defaultView[2]
        )
    ...
    def turnBoard(self, side="auto"):
        ...
        if side == "black":
            ...
            self.world.setH(180)
            t_x = self.worldData["worldPosition"][0] - (
                2 * self.worldData["worldPosition"][0]
            )
    ...
    def initMessageSystem(self, callBackBundle=None):
        ...
        for t_x in self.XPU.Parser["XML_Values"]["Messages_Values"]:
            self.messageLibrary[t_x] = self.XPU.Parser["XML_Values"]["Messages_Values"][
                t_x
            ]
            if callBackBundle != None:
                if t_x in callBackBundle:
                    self.accept(
                        self.messageLibrary[t_x],
                        callBackBundle[t_x][0],
    ...
    def generateLogFile(self):
        ...
        movesXGU.createValue(
            "value", "valueName", "Level", self.logData["level"], "Description"
        )
        movesXGU.createValue(
            "value",
            "valueName",
            "Difficulty",
            self.logData["difficulty"],
            "Description",
        )
    ...
    def eventProcessor(self, eventCode):
        ...
        if eventCode in self.eventCodeLibrary:
    ...
    def followMouseTask(self, task):
        ...
        if self.mouseDownFlag == True:
            ...
            if base.mouseWatcherNode.hasMouse():
                ...
                self.piecePicker.pickerRay.setFromLens(
                    base.camNode, t_mp.getX(), t_mp.getY()
                )
                ...
                t_np = self.world.getRelativePoint(
                    camera, self.piecePicker.pickerRay.getOrigin()
                )
                # same thing with the direction of the ray
                t_nv = self.world.getRelativeVector(
                    camera, self.piecePicker.pickerRay.getDirection()
                )
