...
class EdenMaze(Adam):
    ...
    def __init__(self):  # constructor
        ...
        self.accept(
            "d", self.textPlay.displayText, ["Doc", 30, Vec3(-0.70, 0, 0.50), False]
        )
        self.accept(
            "i", self.textPlay.displayText, ["Game", 30, Vec3(0.05, 0, 0.95), False]
        )
    ...
    def printEvent(self, eventNumber):
        ...
        self.hudBrush.menuList["Status"]["labels"]["Event"]["text"] = t_l
    ...
    def checkCredit(self):
        ...
        if self.Credits <= 0:
            ...
            self.hudBrush.menuList["Status"]["labels"]["Credits"]["text"] = t_y
            # show time every second
            t_g = "TIME: " + self.worldData["localTime"]
            self.hudBrush.menuList["Status"]["labels"]["Time"]["text"] = t_g
    ...
    def setNewFlag(self, flagType):
        ...
        if self.flagCount == 0:
            ...
            if flagType == 0:
                ...
                self.eventsList[5] = "%s%d%s" % (
                    "Red flag used. You have ",
                    (self.flagCount - 1),
                    " flags left",
                )
            elif flagType == 1:
    ...
    def loadMaze(self, mazeXML):
        ...
        # get the scale
        self.blockScale = (
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["scaleX"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["scaleY"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["scaleZ"],
        )
        # get the flag scale
        self.flagScale = (
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["fScaleX"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["fScaleY"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["fScaleZ"],
        )
        # get the extremes
        self.mazeExtremes = (
            self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["extremeX"],
            self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["extremeY"],
        )
        # get dimensions
        self.mazeDimensions = (
            self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["length"],
            self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["width"],
        )
        self.cubeSize = self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["cubeWidth"]
        # get the flag count
        self.flagCount = self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["flagCount"]
        # get the aerial view delay
        self.aerialData = self.mazeXPU.Parser["XML_Values"]["Setup_Values"][
            "aerialDelay"
        ]
        # get the actor delay
        self.actorData = self.mazeXPU.Parser["XML_Values"]["Setup_Values"]["actorDelay"]
        # get special spots
        ...
        for t_h in t_m:
            ...
            if self.mazeXPU.Parser["XML_Values"]["Positions_Values"][t_h] == 0:
                ...
                self.ladyMiranda.baseActor.setPos(
                    t_p[0] - int(self.cubeSize / 2), t_p[1] - int(self.cubeSize / 2), 0
                )
    ...
    def insertCube(self, cubeIndex, deadEnd=False, colorIndex=0):
        ...
        # load the cube
        self.loadGeometry(
            self.blockModel, t_s, self.blockScale, (t_p[0], t_p[1], 0.0), t_x
        )
        # setup collision
    ...
    def actorsBumpHandler(self, targetNode, extraData, entry):
        ...
        if targetNode in self.exitList:
            ...
            if t_d == "Gorilla":
                ...
                if t_l == 0:
                    self.ladyMiranda.baseActor.setPos(
                        self.startingPos[0], self.startingPos[1], self.startingPos[2]
                    )
                    self.printEvent(14)
    ...
    def gameTask(self, task):
        ...
        if task.time == 0.0:
            ...
            # set the new title
            self.hudBrush.menuList["Status"]["labels"]["Dir"][
                "text"
            ] = self.defaultTitle
            return task.cont
        else:
