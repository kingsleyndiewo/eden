...
class EdenLudo(Creation):
    ...
    def loadBoard(self):
        ...
        # get the scale
        t_s = (
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["pScaleX"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["pScaleY"],
            self.mazeXPU.Parser["XML_Values"]["Scale_Values"]["pScaleZ"],
        )
        ...
        for t_x in range(4):
            for t_y in range(4):
                # get the positions
                t_p = (
                    self.boardPositions["start"][t_x][t_y][0],
                    self.boardPositions["start"][t_x][t_y][1],
                    1.5,
                )
                t_m = self.mazeXPU.Parser["XML_Values"]["Players_Values"][
                    "model%d" % t_x
                ]
    ...
    def rollDice(self):
        ...
        if self.sixFlag == False:
            ...
            if t_c == len(self.teams[self.activePlayer]):
                ...
                t_n = "%s player is unlucky %s's turn" % (
                    self.prevPlayer,
                    self.activePlayer,
                )
            ...
            for t_k in range(4):
                if (
                    self.teams[self.activePlayer][t_k][1] == False
                    and self.teams[self.activePlayer][t_k][3] == False
                ):
                    break
                else:
                    t_j += 1
    ...
    def printHud(self, path, message):
        ...
        self.hudBrush.menuList["Status"]["labels"][path]["text"] = str(message)
    ...
    def movePiece(self):
        ...
        if (
            self.sixFlag == True
            and self.teams[self.activePlayer][self.playerIndex][1] == True
        ):
            ...
            self.sixFlag == False
            and self.teams[self.activePlayer][self.playerIndex][1] == False
        ):
            t_l = [self.greenPath, self.yellowPath, self.redPath, self.bluePath]
