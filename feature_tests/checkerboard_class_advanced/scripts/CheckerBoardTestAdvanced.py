...
# File name: CheckerBoardTestAdvanced.py
...
# --------------------------------------------------
class CheckerBoardTestAdvanced(CheckerBoard):
    "Extending the CheckerBoard class for the basic 2-player checkers game"
    # ------------------CONSTRUCTOR------------------------
    # ----------------------------------------------------
    def __init__(self):  # constructor
        CheckerBoard.__init__(
            self, customPRC={"fullscreen": False}
        ...
        self.textPlay.loadTextLine(
            "CheckerBoard Advanced Test. Powered by Funtrench", self.textKeys[0]
        )
        # instructions
        self.textPlay.loadTextLine("Press ESCAPE to quit", self.textKeys[1])
        self.textPlay.loadTextLine(
            "Press B/W to turn board to your side", self.textKeys[2]
        )
        self.textPlay.loadTextLine(
            "Use the LMB to select and move a piece", self.textKeys[3]
        )
        ...
        # setup key bindings
        self.accept("s", self.screenCapture, ["../CheckerBoardAdvanced_"])
        self.accept(
