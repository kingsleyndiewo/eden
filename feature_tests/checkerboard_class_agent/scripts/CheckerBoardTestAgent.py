...
# File name: CheckerBoardTestAgent.py
...
# Class.
# Class definition for the CheckerBoardTestAdvanced class
# --------------------------------------------------
class CheckerBoardTestAgent(CheckerBoard):
    "Extending the CheckerBoard class for a basic AI showcase game"

    # ------------------CONSTRUCTOR------------------------
    # ----------------------------------------------------
    def __init__(self):  # constructor
        CheckerBoard.__init__(
            self, ignorePicker=True, customPRC={"fullscreen": False}
        ...
        self.textPlay.loadTextLine(
            "CheckerBoard Agent Test. Powered by Funtrench", self.textKeys[0]
        )
        # instructions
        self.textPlay.loadTextLine("Press ESCAPE to quit", self.textKeys[1])
        self.textPlay.loadTextLine("K: Start an AI game", self.textKeys[2])
        self.textPlay.loadTextLine("H: Change board skin", self.textKeys[3])
        self.textPlay.loadTextLine("I: Show game information", self.textKeys[4])
        self.textPlay.loadTextLine("D: Show documentation", self.textKeys[5])
        ...
        # manual text
        t_n = (
            self.gameMVC.mvcStructure["tier_resource"]["text"] + "/CheckerBoardTest.txt"
        )
        ...
        # setup key bindings
        self.accept("s", self.screenCapture, ["../CheckerBoardAgent_"])
        self.accept(
            "d", self.textPlay.displayText, ["Doc", 30, Vec3(-1.00, 0, 0.35), False]
        )
        self.accept(
            "i", self.textPlay.displayText, ["Game", 30, Vec3(0.20, 0, 0.95), False]
        )
        self.accept("c", self.textPlay.clearScreen)
        self.accept("k", self.setupAutoGame)
        self.accept("h", self.changeSkin)
        # initialize message system
        t_d = {
            "pieceMove": (self.eventProcessor, ["M"]),
