...
# Desc: Test Project for the Adam Class
...
# Class definition for the AdamTest class
# --------------------------------------------------
class AdamTest(Adam):
    "Extending the Adam class for two actors in a world"
    ...
    def __init__(self):  # constructor
        ...
        # title
        self.textPlay.loadTextLine("Adam Test. Powered by Funtrench", self.textKeys[0])
        # instructions
        ...
        self.textPlay.loadTextLine("CTRL: Hold to run.", self.textKeys[3])
        ...
        self.sirRalphPosInterval1 = self.sirRalph.baseActor.posInterval(
            10, Point3(0, -15, 0), startPos=Point3(0, 35, 0)
        )
        self.sirRalphPosInterval2 = self.sirRalph.baseActor.posInterval(
            10, Point3(0, 35, 0), startPos=Point3(0, -10, 0)
        )
        self.sirRalphHprInterval1 = self.sirRalph.baseActor.hprInterval(
            3, Point3(180, 0, 0), startHpr=Point3(0, 0, 0)
        )
        self.sirRalphHprInterval2 = self.sirRalph.baseActor.hprInterval(
            3, Point3(0, 0, 0), startHpr=Point3(180, 0, 0)
        )
        # create and play the sequence that coordinates the intervals
        self.sirRalphPace = Sequence(
            self.sirRalphPosInterval1,
            self.sirRalphHprInterval1,
            self.sirRalphPosInterval2,
            self.sirRalphHprInterval2,
            name="sirRalphPace",
        )
        ...
        self.accept("s", self.screenCapture, ["../Adam_"])
        self.accept(
            "d", self.textPlay.displayText, ["Doc", 30, (-0.70, 0, 0.50), False]
        )
        self.accept(
