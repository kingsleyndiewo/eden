...
# Desc: Advanced II Test Project for the Eve Class
# File name: EveTestGMS.py
...
# Class definition for the EveTestGMS class
# --------------------------------------------------
class EveTestGMS(Eve):
    ...
    def __init__(self):  # constructor
        ...
        self.textPlay.loadTextLine(
            "Eve Test GMS. Powered by Funtrench", self.textKeys[0]
        )
        # instructions
        self.textPlay.loadTextLine("Press ESCAPE to quit", self.textKeys[1])
        self.textPlay.loadTextLine(
            "Right-Click: Select(Activate) Actor", self.textKeys[2]
        )
        self.textPlay.loadTextLine(
            "Arrow Keys: Control Selected Actor", self.textKeys[3]
        )
        ...
        self.accept("s", self.screenCapture, ["../EveGMS_"])
        self.accept(
            "d", self.textPlay.displayText, ["Doc", 30, (-0.70, 0, 0.45), False]
        )
        self.accept(
    ...
    def pickTool(self):
        ...
        self.selectInventoryItem(
            "Miranda", self.toolBoxLists["Miranda"][self.cycleCount]
        )
        self.cycleCount += 1
        ...
        if task.time == 0.0:
            ...
            if self.edenVisuals.virginFlag == True:
                if self.videoDone == False:
                    ...
                    # capture a movie
                    # self.recordSIVideo(4.0, 15, '../crazy/', 'movie', \
                    #    'jpg', 4)
                    return task.done
            else:
                # all other frames
    ...
    def altimeterTask(self, task):
        ...
        t_s = "%s%f%s" % ("Altitude: ", self.currentAvatar.baseActor.getZ(), " metres")
