...
# Desc: Spin Test Project for the Genesis Class
# File name: GenesisTestSpin.py
...
# Class with unmoused camera. Spin task activated.
# Class definition for the GenesisTestSpin class
# --------------------------------------------------
class GenesisTestSpin(Genesis):
    "Extending the Genesis class for an actor in basic world"
    # ------------------CONSTRUCTOR------------------------
    # ----------------------------------------------------
    def __init__(self):  # constructor
        Genesis.__init__(
            self, self.starterTask, customPRC={"fullscreen": False}
        ...
        # title
        self.textPlay.loadTextLine(
            "Genesis Test Spin. Powered by Funtrench", self.textKeys[0]
        )
        # instructions
        ...
        self.textPlay.loadTextLine("M/N: Toggle world spin", self.textKeys[5])
        ...
        self.ladyEvePosInterval1 = self.ladyEve.baseActor.posInterval(
            10, Point3(0, -15, 0), startPos=Point3(0, 30, 0)
        )
        self.ladyEvePosInterval2 = self.ladyEve.baseActor.posInterval(
            10, Point3(0, 30, 0), startPos=Point3(0, -15, 0)
        )
        self.ladyEveHprInterval1 = self.ladyEve.baseActor.hprInterval(
            2, Point3(180, 0, 0), startHpr=Point3(0, 0, 0)
        )
        self.ladyEveHprInterval2 = self.ladyEve.baseActor.hprInterval(
            2, Point3(0, 0, 0), startHpr=Point3(180, 0, 0)
        )
        # create and play the sequence that coordinates the intervals
        self.ladyEvePace = Sequence(
            self.ladyEvePosInterval1,
            self.ladyEveHprInterval1,
            self.ladyEvePosInterval2,
            self.ladyEveHprInterval2,
            name="ladyEvePace",
        )
        ...
        self.accept("m", self.spinCamera, [12])
        self.accept("n", self.stopCameraSpin)
        self.accept("s", self.screenCapture, ["../GenesisSpin_"])
        self.accept(
            "d", self.textPlay.displayText, ["Doc", 30, (-1.00, 0, 0.35), False]
        )
        self.accept(
            "i", self.textPlay.displayText, ["Game", 30, (0.05, 0, 0.95), False]
        )
    ...
    def cubeAlter(self, valueChange, valueType):
        ...
        if valueType == "X":
            self.objectStore["cube"][0].setX(
                self.objectStore["cube"][0].getX() + valueChange
            )
        elif valueType == "Y":
            self.objectStore["cube"][0].setY(
                self.objectStore["cube"][0].getY() + valueChange
            )
        elif valueType == "Z":
            self.objectStore["cube"][0].setZ(
                self.objectStore["cube"][0].getZ() + valueChange
            )
    ...
    def genesisCameraTask(self, task):
        ...
        base.camera.setPos(
            self.ladyEve.baseActor.getX() + self.camPos[0],
